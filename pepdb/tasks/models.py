...
class AbstractTask(models.Model):
    user = models.ForeignKey(
        User, verbose_name=_("Користувач"),
        blank=True, null=True)

    timestamp = models.DateTimeField(
        verbose_name=_("Створено"), auto_now_add=True)
...
class BeneficiariesMatching(AbstractTask):
    ...
    company_key = models.CharField(
        _("Ключ компанії"), max_length=500)

    person = models.IntegerField(_("Власник в реєстрі PEP"))
    person_json = JSONField(verbose_name=_("Власник в реєстрі PEP"), null=True, dump_kwargs=dump_kwargs)
    ...
    pep_company_information = JSONField(_("Записи компанії, згруповані разом"), dump_kwargs=dump_kwargs)
    candidates_json = JSONField(verbose_name=_("Кандидати на матчінг"), null=True, dump_kwargs=dump_kwargs)
    edrpou_match = models.CharField(
        _("Знайдена компанія"), max_length=15, null=True, blank=True)
...
class EDRMonitoring(AbstractTask):
    ...
    pep_company_json = JSONField(verbose_name=_("Компанія де ПЕП є керівником"), null=True)
    ...
    applied = models.BooleanField(verbose_name=_("Застосовано"), default=False, db_index=True)
...
class TerminationNotice(AbstractTask):
    ...
    termination_date_ceiled = models.DateField(
        _("Дата припинення статусу ПЕП, округлена вгору"),
        blank=True, null=True
    )
    ...
    pep_name = models.CharField(
        _("Прізвище"), max_length=200, null=True, blank=True)
...
class AdHocMatch(AbstractTask):
    ...
    dataset_id = models.CharField(_("Походження датасету"), max_length=200, null=True, blank=True)
    name_match_score = models.IntegerField(_("Ступінь співпадіння"))
    name_in_dataset = models.CharField(
        _("ПІБ з датасету"), max_length=200, null=True, blank=True)

    applied = models.BooleanField(
        _("Матч було застосовано"), default=False
    )
...
class WikiMatch(AbstractTask):
    ...
    matched_json = DjangoJSONField(verbose_name=_("Сутності, знайдені у вікі"), null=True)
    wikidata_id = models.CharField(_("Коректний WikiData ID"), max_length=50, blank=True)
...
class SMIDACandidate(AbstractTask):
    ...
    smida_edrpou = models.CharField(_("Код компанії"), max_length=15, null=True, blank=True)
    smida_company_name = models.CharField(_("Назва компанії"), max_length=200, null=True, blank=True)
    ...
    smida_parsed_name = models.CharField(_("'Чистий 'ПІБ зі звіту"), max_length=200, blank=True)
    ...
    smida_position_body = models.CharField(_("Орган"), max_length=2, choices=POSITION_BODIES)
    smida_position_class = models.CharField(_("Рівень"), max_length=1, choices=POSITION_CLASSES)
