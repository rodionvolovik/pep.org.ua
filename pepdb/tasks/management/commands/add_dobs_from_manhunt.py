...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for rec in AdHocMatch.objects.filter(
                status="a", dataset_id="wanted_ia").prefetch_related("person").nocache():
            ...
            if rec.person.dob is None or rec.person.dob_details > 0:
                ...
                if dob != rec.person.dob:
                    self.stderr.write("DOB of the person {} in pep db and wanted DB differs ({} vs {})".format(
                        rec.person, rec.person.dob, dob
                    ))
                    failed += 1
                else:
                    self.stdout.write("Not updating DOB of the person {} in pep db and wanted DB ({} vs {})".format(
                        rec.person, rec.person.dob, dob
                    ))
