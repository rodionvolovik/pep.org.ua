...
class Command(BaseCommand):
    ...
    def get_latest_declaration_record(self, ownership):
        ...
        if ownership.pep_company_information:
            return sorted(ownership.pep_company_information, key=key_fun, reverse=True)[
                0
            ]
        ...
        for ownership in BeneficiariesMatching.objects.filter(status="m"):
            ...
            for d in ownership.declarations:
                ...
                conn, conn_created = self.conn_importer.get_or_create_from_declaration(
                    person,
                    company,
                    most_recent_record.get("link_type", "Бенефіціарний власник"),
                    decl,
                    save_it,
                )
        ...
        for ownership in BeneficiariesMatching.objects.filter(status="y"):
            if len(ownership.candidates_json) != 1:
                self.stderr.write(
                    ...
                    % (len(ownership.candidates_json), ownership.company_key)
                )
                continue
            ...
            try:
                ...
                self.stderr.write(
                    ...
                    % (ownership.candidates_json[0]["id"], ownership.company_key)
                )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            "Creation failed: %s, creation successful: %s"
