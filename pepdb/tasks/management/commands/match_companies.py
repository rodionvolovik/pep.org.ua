...
class Command(BaseCommand):
    help = ('Find candidates for matching between companies in DB and '
    ...
    def search_me(self, company, fuzziness=1, candidates=10):
        if company["edrpou"]:
            res = EDRPOU.search().query(
                "term", edrpou=company["edrpou"].lstrip("0"))
        else:
            should = [
                Q(
                    "match",
    ...
    def handle(self, *args, **options):
        for company in Company.objects.values(
            ...
            company["heads"] = [
                " ".join(x) for x in Person2Company.objects.filter(
