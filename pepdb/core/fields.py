from django import forms
from django.conf import settings
from django.core.urlresolvers import reverse_lazy
from redactor.widgets import RedactorEditor as BaseRedactorEditor
from redactor.fields import RedactorField as BaseRedactorField


GLOBAL_OPTIONS = getattr(settings, "REDACTOR_OPTIONS", {})


class RedactorEditor(BaseRedactorEditor):
    def __init__(self, *args, **kwargs):
        upload_to = kwargs.pop("upload_to", "")

        self.options = dict(getattr(settings, "REDACTOR_OPTIONS", {}))
        self.options.update(kwargs.pop("redactor_options", {}))

        if kwargs.pop("allow_file_upload", True):
            self.options["fileUpload"] = reverse_lazy(
                "redactor_upload_file", kwargs={"upload_to": upload_to}
            )
        if kwargs.pop("allow_image_upload", True):
            self.options["imageUpload"] = reverse_lazy(
                "redactor_upload_image", kwargs={"upload_to": upload_to}
            )

        widget_attrs = {"class": "redactor-box-inactive"}
        widget_attrs.update(kwargs.get("attrs", {}))
        widget_attrs.update({"data-redactor-options": self.options})

        kwargs["attrs"] = widget_attrs
        super(BaseRedactorEditor, self).__init__(*args, **kwargs)

    def _media(self):
        _min = "" if settings.DEBUG else ".min"
        js = (
            "js/admin/jquery.redactor.init.js",
            "redactor/redactor{}.js".format(_min),
            "redactor/langs/{}.js".format(self.options.get("lang", "en")),
        )

        if "plugins" in self.options:
            plugins = self.options.get("plugins")
            for plugin in plugins:
                js = js + ("redactor/plugins/{}.js".format(plugin),)

        css = {
            "all": ("redactor/redactor{}.css".format(_min), "redactor/django_admin.css")
        }
        return forms.Media(css=css, js=js)

    media = property(_media)


class RedactorField(BaseRedactorField):
    def __init__(self, *args, **kwargs):
        options = kwargs.pop("redactor_options", {})
        upload_to = kwargs.pop("upload_to", "")
        allow_file_upload = kwargs.pop("allow_file_upload", True)
        allow_image_upload = kwargs.pop("allow_image_upload", True)
        self.widget = RedactorEditor(
            redactor_options=options,
            upload_to=upload_to,
            allow_file_upload=allow_file_upload,
            allow_image_upload=allow_image_upload,
        )
        super(BaseRedactorField, self).__init__(*args, **kwargs)
