...
class PersonAdmin(nested_admin.NestedModelAdminMixin, TranslationAdmin):
    ...
    list_display = localized_fields(["last_name", "first_name", "patronymic"]) + [
        "is_pep",
        "dob",
        "dob_details",
        "type_of_official",
    ...
    search_fields = localized_fields(["last_name", "first_name", "patronymic"]) + [
        "names"
    ]
    ...
    fieldsets = [
        ...
        (_("Остання зміна"), {"fields": ["last_change", "last_editor"]}),
    ]
...
class CountryAdmin(TranslationAdmin):
    list_display = localized_fields(["name"], langs=settings.LANGUAGE_CODES) + [
        "iso2",
        "iso3",
        "is_jurisdiction",
        "order",
...
class DocumentAdmin(TranslationAdmin):
    ...
    list_display = (
        ["pk"] + localized_fields(["name"]) + ["link", "uploader", "uploaded", "doc"]
    )
...
class DeclarationAdmin(DeclarationBaseAdmin):
    ...
    def store_relatives(self, request):
        ...
        self.message_user(
            request,
            _("%s осіб та %s зв'язків було створено.")
            % (persons_created, connections_created),
        )

        self.message_user(
            request,
            _("%s осіб та %s зв'язків було оновлено.")
