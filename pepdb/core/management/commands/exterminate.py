...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        fired_persons = Person.objects.filter(reason_of_termination__isnull=False).values_list("pk", flat=True)
        ...
        for family_member in family_members.iterator():
            ...
            to_persons_cnt = Person2Person.objects.prefetch_related("to_person") \
                .filter(
            ...
            from_persons_cnt = Person2Person.objects.prefetch_related("from_persons") \
                .filter(
        ...
        self.stdout.write("Found {} true family members".format(len(true_family_members)))
        ...
        for pep in peps_by_connections.iterator():
            ...
            to_pep_persons_cnt = Person2Person.objects.prefetch_related("to_person") \
                .filter(
                    from_person=pep,
                    to_person__type_of_official__in = [1, 2, 3],
                ).exclude(
            ...
            from_pep_persons_cnt = Person2Person.objects.prefetch_related("from_persons") \
                .filter(
                    to_person=pep,
                    from_person__type_of_official__in = [1, 2, 3],
                ).exclude(
            ...
            to_family_members_cnt = Person2Person.objects.prefetch_related("to_person") \
                .filter(
                    from_person=pep,
                    to_person__pk__in=true_family_members
                ).count()

            from_family_members_cnt = Person2Person.objects.prefetch_related("from_persons") \
                .filter(
                    to_person=pep,
                    from_person__pk__in=true_family_members
                ).count()
            ...
            to_fired_persons = list(Person2Person.objects.prefetch_related("to_person") \
                .filter(
                    from_person=pep,
                    to_person__type_of_official__in = [1, 2, 3],
                    to_person__pk__in=(fired_persons)
            ...
            from_fired_persons = list(Person2Person.objects.prefetch_related("from_persons") \
                .filter(
                    to_person=pep,
                    from_person__type_of_official__in = [1, 2, 3],
                    from_person__pk__in=(fired_persons)
            ...
            last_survivor = None
            for fired_pep in Person.objects.filter(pk__in = to_fired_persons + from_fired_persons):
                if not fired_pep.terminated:
                    continue
                if not last_survivor:
                    last_survivor = fired_pep
                elif (ceil_date(fired_pep.termination_date, fired_pep.termination_date_details) >
                        ceil_date(last_survivor.termination_date, last_survivor.termination_date_details)):
                    last_survivor = fired_pep

            if last_survivor is None:
                self.stdout.write(
                    "All the peps by position connected to {} resigned but they are still peps, so skipping".format(pep)
                )
                continue
            ...
            TerminationNotice.objects.get_or_create(
                ...
                defaults={
                    ...
                    "termination_date_ceiled": ceil_date(last_survivor.termination_date, last_survivor.termination_date_details),
                    "pep_position": pep_position,
                    "comments": "Припинив бути ПЕПом з {}, тому що остання пов'язана особа {} припинила бути ПЕПом з причини '{}'".format(
                        render_date(last_survivor.termination_date, last_survivor.termination_date_details),
