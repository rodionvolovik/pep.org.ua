...
class Command(BaseCommand):
    ...
    special_replacements = {"ий$": ["y", "i"], "ий\s": ["y ", "i "], "кс": ["x"]}

    def __init__(self, *args, **kwargs):
        self.ru_translations = {}

        for t in Ua2RuDictionary.objects.all():
            self.ru_translations[t.term.lower()] = filter(
    ...
    def transliterate(
        self, person_last_name, person_first_name, person_patronymic, lang
    ):
        first_names = []
        last_names = []
        patronymics = []
        ...
        if lang == "uk":
            ...
            if person_first_name.lower() in self.ru_translations and is_cyr(
                person_first_name
            ):
            ...
            if person_last_name.lower() in self.ru_translations and is_cyr(
                person_last_name
            ):
            ...
            if person_patronymic.lower() in self.ru_translations and is_cyr(
                person_patronymic
            ):
        ...
        translated = [
            (l, f, p) for f in first_names for p in patronymics for l in last_names
        ]
                ...
                for sc, replacements in self.special_cases.items():
                    if sc in n:
                        for repl in replacements:
                            ...
                            result.add(
                                translit(self.get_name(optional_n), UkrainianKMU)
                            )
            ...
            for sc_rex, replacements in self.special_replacements.items():
                if re.search(sc_rex, name, flags=re.I | re.U):
                    for repl in replacements:
                        optional_n = re.sub(sc_rex, repl, name, flags=re.I | re.U)
                        result.add(
                            translit(title(optional_n), RussianInternationalPassport)
                        )
            ...
            for sc, replacements in self.special_cases.items():
                if sc in n:
                    for repl in replacements:
                        ...
                        result.add(
                            translit(
                                self.get_name(optional_n), RussianInternationalPassport
                            )
                        )
        ...
        for person in Person.objects.all().nocache().iterator():
            ...
            if aka:
                for aka_name in filter(None, aka.split("\n")):
                    aka_last_name, aka_first_name, aka_patronymic, _ = parse_fullname(
                        aka_name
                    )
                    names |= self.transliterate(
                        aka_last_name,
